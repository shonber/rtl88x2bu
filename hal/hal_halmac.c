...
int rtw_halmac_pinmux_set_func(struct dvobj_priv *d, u8 gpio_func)
{
	struct _HALMAC_ADAPTER *halmac;
	struct _HALMAC_API *api;
	enum _HALMAC_RET_STATUS status;
	...
	status = api->halmac_pinmux_set_func(halmac, gpio_func);
	if (status != HALMAC_RET_SUCCESS)
		return -1;
...
int rtw_halmac_pinmux_wl_led_mode(struct dvobj_priv *d, u8 wlled_mode)
{
	struct _HALMAC_ADAPTER *halmac;
	struct _HALMAC_API *api;
	enum _HALMAC_RET_STATUS status;
	...
	status = api->halmac_pinmux_wl_led_mode(halmac, wlled_mode);
	if (status != HALMAC_RET_SUCCESS)
		return -1;
...
void rtw_halmac_pinmux_wl_led_sw_ctrl(struct dvobj_priv *d, u8 led_on)
{
	struct _HALMAC_ADAPTER *halmac;
	struct _HALMAC_API *api;
	...
	api->halmac_pinmux_wl_led_sw_ctrl(halmac, led_on);
